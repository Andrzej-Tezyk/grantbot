...
@router_history.get("/{company_id}", response_model=list[HistoryItem], tags=["History"])
...
@router_history.get(
    "/request/{request_id}", response_model=HistoryItem, tags=["History"]
)
async def get_request(request_id: str, session: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        history_item = await HistoryService.get_request_by_id(
            session=session, request_id=request_uuid
        )
